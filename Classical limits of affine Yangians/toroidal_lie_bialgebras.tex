\section{Toroidal Lie (bi)algebras}
    \subsection{(Extended) toroidal Lie algebras}
        Let us begin by recalling the following description of universal central extensions (UCEs) of current algebras, i.e. of Lie algebras of the form $\bar{\g} \tensor A$ wherein $A$ is a commutative algebra, due to Kassel. For a self-contained but comprehensive account of the following facts, we direct the reader to \cite[Section 2.2]{msc_thesis_gamma_extended_toroidal_lie_algebras}.
        
        First of all, for brevity, let us write $xf$ to denote the pure tensors $x \tensor f \in \bar{\g} \tensor A$. On $\bar{\g} \tensor A$, the Lie algebra structure is given by:
            \begin{equation} \label{equation: current_algebra_brackets}
                [xf, yg]_{\bar{\g} \tensor A} := [x, y]_{\bar{\g}} fg
            \end{equation}
        It is worth noting, that with this Lie bracket, $\bar{\g} \tensor A$ is a perfect Lie algebra\footnote{A Lie algebra is said to be perfect if it is equal to its derived subalgebra.} as a consequence of the simplicity of the Lie algebra $\bar{\g}$ that we assumed from before. This fact is important because Lie algebras are perfect if and only if they admit UCEs (see \cite[Lemma 1.10]{garland_arithmetics_of_loop_groups}); for a perfect Lie algebra $\a$, we denote its UCE by $\uce(\a)$. It is also desirable to have a description of the structure of $\toroidal_A$, and since:
            $$\toroidal_A := \uce(\bar{\g} \tensor A) \cong ( \bar{\g} \tensor A ) \oplus^{\e} \z_A$$
        wherein $\e: \bigwedge^2 (\bar{\g} \tensor A) \to \z_A$ is the universal Lie $2$-cocycle with values in the centre $\z_A := \z( \toroidal_A )$, it suffices to give a more detailed description of the centre. In \cite{kassel_universal_central_extensions_of_lie_algebras}, Kassel demonstrated via the use of cyclic cohomology, that the centre can be given via:
            $$\z_A \cong \bar{\Omega}^1_A := \Omega^1_A/dA \quad, \quad \e(xf, yg) := (x, y)_{\bar{\g}} g \bar{d}f$$
        ...
        for the image of $df \in \Omega^1_A$ under the canonical quotient map $\Omega^1_A \to \bar{\Omega}^1_A$. In particular, this implies that the non-trivial commutators in $\toroidal_A$ are given by the following formula:
            \begin{equation} \label{equation: toroidal_lie_algebra_commutators}
                \begin{aligned}
                    [xf, yg]_{\toroidal_A} & = [xf yg]_{\bar{\g} \tensor A} + \e(xf, yg)
                    \\
                    & = [x, y]_{\bar{\g}} fg + (x, y)_{\bar{\g}} g \bar{d}f
                \end{aligned}
                \quad, \quad
                x, y \in \bar{\g}, f, g \in A
            \end{equation}

        More can be said when $A$ is graded by some abelian group $Z$, say $A := \bigoplus_{n \in Z} A_n$. In this case, $\bar{\g} \tensor A$ and $\Omega^1_A$ carry an induced $Z$-grading given by:
            $$\bar{\g} \tensor A = \bigoplus_{n \in Z} (\bar{\g} \tensor A)_n \quad, \quad (\bar{\g} \tensor A)_n := \bar{\g} \tensor A_n$$
        ...
        At the same time, for a distinguished element $n_0 \in Z$, one can define a symmetric, non-degenerate, and invariant bilinear form on $\bar{\g} \tensor A$ given by:
            \begin{equation} \label{equation: current_algebra_pairing}
                (x f, y g)_{\bar{\g} \tensor A} := (x, y)_{\bar{\g}} \1_{ \deg f + \deg g, n_0 }
            \end{equation}
        ...
        This dengeracy of the bilinear form $(\cdot, \cdot)_{\toroidal_A}$ creates an obstacle for us, however. The theory of Manin triples (see e.g. \cite{etingof_kazhdan_quantisation_1}) tells us that it is necessary to have a non-degenerate, symmetric, and invariant bilinear form, yet we know that $(\cdot, \cdot)_{\toroidal_A}$ is \textit{degenerate}. That said, we can enlarge $\toroidal_A$ in such a way that the extra elements pair non-degenerately with those of $\z_A$, which are the very ones causing the degeneracy issue for $(\cdot, \cdot)_{\toroidal_A}$. In subsection \ref{subsection: toroidal_lie_bialgebras} below, we shall show that $\toroidal_A$ can be acted upon by a certain Lie subalgebra $\d_A$ of the vector field Lie algebra $\der(A)$, so as to give rise to Lie algebra extensions:
            $$0 \to \toroidal_A \to \extendedtoroidal_A \to \d_A \to 0$$
        In particular, this implies that $\toroidal_A$ is a Lie subalgebra of the enlargement $\extendedtoroidal_A$.
        
        If we let $(\cdot, \cdot)_{\extendedtoroidal_A}$ denote the extension of $(\cdot, \cdot)_{\toroidal_A}$ to $\extendedtoroidal_A$, then per the outline above, the former will have to pair $\d_A$ non-degenerately with $\z_A$. In order to construct such a bilinear form, let us take:
            $$\d_A := \z_A^{\star}$$
        ...
        We shall let the grading abelian group be:
            $$Z := \Z^2$$
        \begin{remark}
            The cases that we most interested in are when the distinguished element $n_0 := (r_0, s_0) \in \Z^2$ are:
                $$(r_0, s_0) = (0, -1) \quad, \quad \text{and} \quad, \quad (r_0, s_0) = (0, 0)$$
            The reason for this is that in these cases, $\toroidal_A$ is to be the classical limits -respectively - of:
            \begin{enumerate}
                \item the double Yangian associated to the affine Kac-Moody algebra $\g$ associated to $\bar{\g}$ (i.e. the affinisation of $\bar{\g}$), and
                \item the quantum toroidal algebra associated to $\bar{\g}$ (i.e. the quantum affinisation associated to $\g$).
            \end{enumerate}
            We expect the latter to somehow \say{degenerate} to the former\footnote{Not quite, since quantum toroidal algebras should degenerate to affine Yangians, not affine double Yangians, but we lack the language to phrase this rigorously at the moment.}, so it is important to analyse the relationship between their classical limits as well.
        \end{remark}

        Let us now recall the construction of $\d_A$, and then of $\extendedtoroidal_A$. Our main reference is \cite{msc_thesis_gamma_extended_toroidal_lie_algebras}. First of all, let us describe a specific basis for $\z_A$. This was first done in \cite{moody_rao_yokonuma_vertex_representations_of_toroidal_lie_algebras}, but as many details were omitted in \textit{loc. cit.} (presumably because the material is well-known to experts), and for notational consistency, let us follow the presentation in \cite[Subsection 2.3.3]{msc_thesis_gamma_extended_toroidal_lie_algebras}. Through the induced $\Z^2$-grading on $\Omega^1_A$, and hence also on $\bar{\Omega}^1_A$, one can write:
        ...
        can be written in terms of the basis vectors $K_{r, s}, c_v, c_t$ in the following manner:
            $$v^n t^q \bar{d}(v^m t^p) = (mq - np) K_{m + n, p + q} + \1_{(m, p) + (n, q), (0, 0)} ( m c_v + p c_t )$$
        ...
        \begin{remark}
            Note that the linear map $\gamma: A \to \bbC$ helps us recover the bilinear form \eqref{equation: current_algebra_pairing} in the following manner:
                $$(xf, yg)_{\bar{\g} \tensor A} = (x, y)_{\bar{\g}} \gamma(fg) \quad, \quad x, y \in \bar{\g}, f, g \in A$$
        \end{remark}
            ...
            \begin{enumerate}
                ...
                Its elements are given in terms of the partial derivatives $\del_v := \frac{\del}{\del v}$ and $\del_t := \frac{\del}{\del t}$ by:
                    \begin{equation} \label{equation: div_zero_vector_fields_basis}
                        \begin{gathered}
                            D_{r, s} := -s v^{-r + r_0 + 1} t^{-s + s_0} \del_v + r v^{-r + r_0} t^{-s + s_0 + 1} \del_t
                            \\
                            D_v := -v^{r_0 + 1} t^{s_0} \del_v \quad, \quad D_t := -v^{r_0} t^{s_0 + 1} \del_t
                        \end{gathered}
                    \end{equation}
        ...
        \begin{proposition}[A $\d_A$-action on $\toroidal_A$ by Lie derivatives] \label{prop: vector_field_action_on_toroidal_lie_algebras}
            ...
            by Lie derivations, given for all $D \in \der(A)$ by the following formulae:
                \begin{equation} \label{equation: vector_field_action_on_toroidal_lie_algebras}
                    \begin{gathered}
                        \rho(D)( xf ) := x D(f) \quad, \quad x \in \bar{\g}, f \in A
                        \\
                    ...
                    \begin{gathered}
                        \begin{cases}
                            ...
                            \begin{aligned}
                                ...
                                \\
                                & + \1_{(r - r_0, s - s_0), (a, b)} \left( r c_v + s c_t \right)
                            \end{aligned}
                            \\
    ...
    \subsection{Lie bialgebra structures on (extended) toroidal Lie algebras} \label{subsection: toroidal_lie_bialgebras}
        ...
        \begin{question} \label{question: classical_limit_of_affine_yangians}
            What is the classical limit of the coproduct structures on the Yangian of the untwisted affine Kac-Moody algebra $\g$ that were constructed in \cite{guay_nakajima_wendlandt_affine_yangian_coproduct} and in \cite{ueda_coproduct_for_the_yangian_of_twisted_affine_type_A2}?
        \end{question}
        Issues of when such coproduct structures are defined aside (there are certain complications in type $\sfA_1^{(1)}$) and when the affine Yangian is even a flat deformation to begin with (so far, this is known to be true only when $\g$ is simply laced), so that we can be assured that inquiring about the classical limit is even a well-posed question to begin with, we ought to begin answering question \ref{question: classical_limit_of_affine_yangians} by constructing some Lie bialgebra structure which can be nominated as a candidate for being the classical limit of the affine Yangian.
        
        Constructing Lie bialgebras begins with constructing Manin triples, and let us inspiration from the construction of the finite-type Yangian associated to $\bar{\g}$. Recall that this is the\footnote{The uniqueness of the Yangian has recently been confirmed in \cite{gautam_wendlandt_xu_yangian_uniqueness}.} quantisation of a certain Lie bialgebra structure on the current algebra $\bar{\g}[t] := \bar{\g} \tensor \bbC[t]$. This Lie bialgebra structure comes from the $\Z$-graded Manin triple:
            \begin{equation} \label{equation: finite_yangian_manin_triples}
                \left( \bar{\g}[t^{\pm 1}], \bar{\g}[t], t^{-1}\bar{\g}[t^{-1}] \right)
            \end{equation}
        wherein:
        \begin{itemize}
            \item the Lie algebra structure on $\bar{\g}[t^{\pm 1}] := \bar{\g} \tensor \bbC[t^{\pm 1}]$ is given as in equation \eqref{equation: current_algebra_brackets}, and $\bar{\g}[t]$ and $t^{-1}\bar{\g}[t^{-1}] := \bar{\g} \tensor t^{-1}\bbC[t^{-1}]$ are Lie subalgebras of $\bar{\g}[t^{\pm 1}]$ in the canonical manner,
            \item the symmetric, non-degenerate, and invariant bilinear form $(\cdot, \cdot)_{\bar{\g}[t^{\pm}]}$ on $\bar{\g}[t^{\pm}]$ is given as in equation \eqref{equation: current_algebra_pairing}, with $Z := \Z$ and $n_0 := -1$, and one checks that the Lie subalgebras $\bar{\g}[t]$ and $t^{-1}\bar{\g}[t^{-1}]$ are (maximally) isotropic with respect to this bilinear form,
            \item $\bar{\g}[t]$ is regarded as a $\Z$-graded Lie algebra by means of:
                $$\deg(x f) := \deg f \quad, \quad \text{$f \in \bbC[t]$ homogeneous}$$
            and then the Lie algebra $t^{-1}\bar{\g}[t^{-1}]$ is regarded as the $\Z$-graded dual of $\bar{\g}[t]$, and we note that $\bar{\g}[t^{\pm 1}] = \bar{\g}[t] \oplus t^{-1}\bar{\g}[t^{-1}]$.
        \end{itemize}
        The Manin triple \eqref{equation: finite_yangian_manin_triples} then gives rise to a quasi-triangular $\Z$-graded Lie bialgebra structure:
            \begin{equation}
                \dot{\delta}_{\rational}^+: \bar{\g}[t] \to \bar{\g}[t_1] \tensor \bar{\g}[t_2^{-1}] 
            \end{equation}
        given by:
            \begin{equation} \label{equation: finite_yangian_cobracket}
                \begin{gathered}
                    \dot{\delta}_{\rational}^+(X) := \left[\Box(X), \dot{\calr}_{\rational} \right] \quad, \quad X \in \bar{\g}[t]
                    \\
                    \dot{\calr}_{\rational} := \bar{\calr} \cdot t_2^{-1} \1\left( \frac{t_2}{t_1} \right)^+ = \frac{\bar{\calr}}{t_2 - t_1}
                \end{gathered}
            \end{equation}
        wherein:
            $$\bar{\calr} := \calr_{\bar{\g}, (\bar{\g}^*)^{\op}}$$
        is the Casimir tensor\footnote{... or in other words, the canonical tensor of the non-degenerate bilinear form $(\cdot, \cdot)_{\Dr(\bar{\g})}$ that helps defines the standard Manin triple $( \Dr(\bar{\g}), \bar{\g}, (\bar{\g}^*)^{\op} )$.} of $\bar{\g}$, and $\1\left( \frac{t_2}{t_1} \right)^+$ is as in equation \eqref{equation: positive_negative_dirac_delta_distributions}.
        ...
        \begin{remark}
            \begin{itemize}
                \item We are to interpret $\dot{\delta}_{\rational}^+(X)$ as in equation \eqref{equation: finite_yangian_cobracket} as a formal sum of its components:
                    $$\dot{\delta}_{\rational}^+(X)_m := [\Box(X), \bar{\calr} \cdot t_1^m t_2^{-m - 1}] \in t_1^m \bar{\g}[t_1] \tensor t_2^{-m - 1} \bar{\g}[t_2^{-1}] \quad, \quad \text{$X \in \bar{\g}[t]$ homogeneous}$$
                \item One can also interpret the cobracket \eqref{equation: finite_yangian_cobracket} as a topological Lie bialgebra structure:
                    $$\dot{\delta}_{\rational}^+: \bar{\g}[t] \to \bar{\g}[t_1] \hattensor \bar{\g}[\![t_2^{-1}]\!]$$
                wherein we regard the RHS as the $(t_1, t_2^{-1})$-adic closure of $\bar{\g}[t_1] \tensor \bar{\g}[t_2^{-1}]$ inside $(\bar{\g} \tensor \bar{\g})[\![t_1, t_2^{-1}]\!]$.
            \end{itemize}
        \end{remark}
        ...
        \begin{remark}[Classical limit of finite-type dual Yangians] \label{remark: finite_dual_yangian_cobracket} 
            From equation \eqref{equation: positive_negative_dirac_delta_distributions}, we know that:
                $$\1\left( \frac{t_2}{t_1} \right)^- = -\1\left( \frac{t_2}{t_1} \right)^+$$
            At the same time, we know that the ($\Z$-graded) dual cobracket:
                $$\dot{\delta}_{\rational}^-: t^{-1}\bar{\g}[t^{-1}] \to t_1^{-1}\bar{\g}[t_1^{-1}] \tensor t_2\bar{\g}[t_2]$$
            on $t^{-1}\bar{\g}[t^{-1}] \cong \bar{\g}[t]^{\star}$ is supposed to be given by:
            ...
            Therefore, the dual cobracket can be given more succinctly by the following formula:
                \begin{equation} \label{equation: finite_dual_yangian_cobracket}
                    \begin{aligned}
                        ...
                        \\
                        & = [\Box(Y), \bar{\calr} \cdot t_2^{-1} \1\left( \frac{t_2}{t_1} \right)^-]
                    \end{aligned}
                    \quad, \quad Y \in t^{-1}\bar{\g}[t^{-1}]
                \end{equation}
        \end{remark}
        ...
        and then let:
            \begin{equation} \label{equation: positive_and_negative_toroidal_lie_algebras}
                \begin{gathered}
                    \toroidal_A^+ := \uce( t^{-s_0 - 1} \bar{\g}[v^{\pm 1}, t] ) \quad, \quad \toroidal_A^- := \uce( t^{s_0} \bar{\g}[v^{\pm 1}, t^{-1}] )
                    \\
        ...
        Via the fact that:
            \begin{equation} \label{equation: positive_and_negative_double_loop_algebras}
                t^{-s_0 - 1} \bar{\g}[v^{\pm}, t] \cong \bigoplus_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \bar{\g} \tensor \bbC v^r t^{s - s_0 - 1}
                \quad, \quad
                t^{s_0} \bar{\g}[v^{\pm}, t^{-1}] \cong \bigoplus_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \bar{\g} \tensor \bbC v^{-r} t^{-s + s_0}
            \end{equation}
        we have the following descriptions for $\z_A^{\pm}$:
            \begin{equation} \label{equation: positive_and_negative_toroidal_centres}
                \z_A^+ \cong \bigoplus_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \bbC K_{r, s} \oplus \bbC c_v
                \quad, \quad
                \z_A^- \cong \bigoplus_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \bbC K_{-r, -s + s_0} \oplus \bbC c_t
            \end{equation}
        with $K_{r, s}, c_v, c_t$ as in equation \eqref{equation: toroidal_centre_basis}. In turn, these explicit realisations for $\z_A^{\pm}$ tell us that:
            \begin{equation} \label{equation: positive_and_negative_div_zero_vector_fields}
                \d_A^+ \cong \bigoplus_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \bbC D_{-r, -s + s_0} \oplus \bbC D_t
                \quad, \quad
                \d_A^- \cong \bigoplus_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \bbC D_{r, s} \oplus \bbC D_v
            \end{equation}
        with $D_{r, s}, D_v, D_t$ as in equation \eqref{equation: div_zero_vector_fields_basis}. Now, even though the constructions above are designed so that:
            $$\extendedtoroidal_A = \extendedtoroidal_A^+ \oplus \extendedtoroidal_A^-$$
        we must caution immediately, that it is \textit{not always true} that the following graded-duality:
            $$(\extendedtoroidal_A^{\pm})^{\star} \cong \extendedtoroidal^{\mp}$$
        via the non-degenerate bilinear form $(\cdot, \cdot)_{\extendedtoroidal_A}$ holds. Rather, it turns out that this is the case if and only if this bilinear form is of degree $(r_0, s_0) = (0, -1)$.
        ...
        \end{proposition}
            \begin{proof}
                Through equation \eqref{equation: positive_and_negative_double_loop_algebras}, we see that:
                    $$( t^{-s_0 - 1} \bar{\g}[v^{\pm}, t] )^{\star} \cong t^{s_0} \bar{\g}[v^{\pm}, t]$$
                ...
                via the non-degenerate bilinear form $(\cdot, \cdot)_{\extendedtoroidal_A}$, with both being consequences of the fact that $\deg D_{r, s} = (-r + r_0, -s + s_0) = (-r, -s - 1)$ and $\deg D_v = \deg D_t = (r_0, s_0) = (0, -1)$ (see corollary \ref{coro: extended_toroidal_bilinear_forms}); therefore, $\extendedtoroidal_A^{\pm}$ are isotropic when regarded as subspaces of $\extendedtoroidal_A$.
            \end{proof}
        \begin{remark}
            Primarily, we are interested in the case when the bilinear form $(\cdot, \cdot)_{\extendedtoroidal_A}$ has degree:
                $$(r_0, s_0) = (0, -1)$$
            because $\toroidal_A$ would then be classical limit of the Yangian of the affine Kac-Moody algebra $\g$. Obviously, there are no differences beyond visual ones between the cases when $(r_0, s_0) = (0, -1) \not = (0, 0)$ and when $(r_0, s_0) = (-1, 0) \not = (0, 0)$, so it is not necessary to consider the latter explicitly. 
            ...
            It would also be interesting to consider cases when $r_0 = s_0 = -1$ simultaneously. In such cases, there will be two distinct families of toroidal Lie (bi)algebras, namely of the forms:
                $$\uce( \bar{\g}[v, t] ) \quad, \quad \uce( v^{-1} \bar{\g}[v^{-1}, t] )$$
        ...
        In light of proposition \ref{prop: extended_toroidal_manin_triples}, let us assume henceforth that:
            $$(r_0, s_0) = (0, -1)$$
        ...
        thereby proving not only that $\ddot{\delta}_{\rational}^+: \extendedtoroidal_A^+ \to \extendedtoroidal_A^+ \tensor \extendedtoroidal_A^+$ is a $\Z^2$-graded Lie bialgebra structure, but moreover, that it is coboundary. To do so, we shall have to demonstrate that $\schouten{ \ddot{\calr}_{\rational}, \ddot{\calr}_{\rational} } \in \left( \bigwedge^3 \extendedtoroidal_A \right)^{\extendedtoroidal_A}$; in fact, it will even be shown that $\schouten{ \ddot{\calr}_{\rational}, \ddot{\calr}_{\rational} } = 0$, thus proving that $\ddot{\delta}_{\rational}^+$ is quasi-triangular. In any event, however, the first step shall be to write down an explicit description of $\ddot{\calr}_{\rational}$. \textit{A priori}, this can be done by first choosing a basis:
            $$\{ J_{r, s} \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \subset \extendedtoroidal_A^+ \quad, \quad \deg J_{r, s} = (r, s)$$
        for the $\Z \x \Z_{\bar{\g}eq 0}$-graded Lie algebra $\extendedtoroidal_A^+$, and then a graded-dual basis:
            $$\{ J_{r, s}^{\star} \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \subset \extendedtoroidal_A^- \quad, \quad \deg J_{r, s}^{\star} = (-r, -s - 1)$$
        (with respect to the non-degenerate bilinear form $(\cdot, \cdot)_{\extendedtoroidal_A}$), and then we shall get:
            \begin{equation} \label{equation: extended_toroidal_classical_r_matrices_general_form}
                \ddot{\calr}_{\rational} = \sum_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} J_{r, s} \tensor J_{r, s}^{\star}
            \end{equation}
        Now, the manner in which we constructed the Manin triple \eqref{equation: extended_toroidal_manin_triples} has already supplied us with such dual bases, namely:
            $$
                \begin{gathered}
                    \{ J_{r, s} \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} := 
                    \begin{aligned}
                        & \{ x_i v^r t^s \}_{1 \leq i \leq \dim \bar{\g}, (r, s) \in \Z \x \Z_{\bar{\g}eq 0}}
                        \\
                        & \cup \left( \{ K_{r, s} \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \cup \{ c_v \} \right)
                        \\
                        & \cup \left( \{ D_{-r, -s - 1} \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \cup \{ D_t \} \right)
                    \end{aligned}
                    \\
                    \{ J_{r, s}^{\star} \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} := 
                    \begin{aligned}
                        & \{ (x_i v^r t^s)^{\star} = x_i^* v^{-r} t^{-s - 1} \}_{1 \leq i \leq \dim \bar{\g}, (r, s) \in \Z \x \Z_{\bar{\g}eq 0}}
                        \\
                        & \cup \left( \{ K_{r, s}^{\star} = D_{r, s} \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \cup \{ c_v^{\star} = D_v \} \right)
                        \\
                        & \cup \left( \{ D_{-r, -s - 1}^{\star} = K_{-r, -s - 1} \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} \cup \{ D_t^{\star} = c_t \} \right)
                    \end{aligned}
                \end{gathered}
            $$
        for some bases $\{x_i\}_{1 \leq i \leq \dim \bar{\g}} \subset \bar{\g}$ and $\{x_i^*\}_{1 \leq i \leq \dim \bar{\g}} \subset \bar{\g}^*$ that are dual to one another with respect to the non-degenerate bilinear form $(\cdot, \cdot)_{\bar{\g}}$; note also, that because we are supposing that $(r_0, s_0) = (0, s_0) \not = (0, 0)$, the graded-dual basis $\{ J_{r, s}^* \}_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}}$ is indeed indexed by $\Z \x \Z_{< 0}$, and hence lies in the $\Z \x \Z_{< 0}$-graded Lie algebra $\extendedtoroidal_A^-$. Using these dual bases, we can then expand upon equation \eqref{equation: extended_toroidal_classical_r_matrices_general_form} as follows:
            $$
                \begin{aligned}
                    & \ddot{\calr}_{\rational}
                    \\
                    & = \sum_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} J_{r, s} \tensor J_{r, s}^{\star}
                    \\
                    & =
                    \begin{aligned}
                        & \sum_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} x_i v_1^r t_1^s \tensor (x_i v_2^r t_2^s)^{\star}
                        \\
                        & + \quad \left( \sum_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} K_{r, s} \tensor K_{r, s}^{\star} + c_{v_1} \tensor c_{v_2}^{\star} \right) + \quad \left( \sum_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} D_{-r, -s - 1} \tensor D_{-r, -s - 1}^{\star} + D_{t_1} \tensor D_{t_2}^{\star} \right)
                    \end{aligned}
                    \\
                    & = 
                    \begin{aligned}
                        & \bar{\calr} \cdot \1\left( \frac{v_2}{v_1} \right) t_2^{-1} \1\left( \frac{t_2}{t_1} \right)^+
                        \\
                        & + \quad \left( \sum_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} K_{r, s} \tensor D_{r, s} + c_{v_1} \tensor D_{v_2} \right) + \quad \left( \sum_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} D_{-r, -s - 1} \tensor K_{-r, -s - 1} + D_{t_1} \tensor c_{t_2} \right)
                    \end{aligned}
                \end{aligned}
            $$
        Using the explicit formulae for the elements $K_{r, s}, c_v, c_t$ (cf. equation \eqref{equation: toroidal_centre_basis}) and $D_{r, s}, D_v, D_t$ (cf. equation \eqref{equation: div_zero_vector_fields_basis}), we get\footnote{Recall also that $\1\left( \frac{t_2}{t_1} \right)^- = -\1\left( \frac{t_2}{t_1} \right)^+$ (cf. equation \eqref{equation: positive_negative_dirac_delta_distributions}).}:
            $$
                \begin{aligned}
                    & \sum_{(r, s) \in \Z \x \Z_{\bar{\g}eq 0}} K_{r, s} \tensor D_{r, s}
                    \\
                    & = \sum_{(r, s) \in \Z \x \Z_{> 0}} K_{r, s} \tensor D_{r, s} + \sum_{(r, s) \in \Z \x \{0\}} K_{0, s} \tensor D_{0, s}
                    \\
                    & = \sum_{(r, s) \in \Z \x \Z_{> 0}} \frac1s v_1^{r - 1} t_1^s \bar{d}v_1 \tensor \left( -s v_2^{-r + 1} t_2^{-s - 1} \del_{v_2} + r v_2^{-r} t_2^{-s} \del_{t_2} \right) + \sum_{r \in \Z} \left( -\frac1r v_1^r t_1^{-1} \bar{d}t_1 \right) \tensor r v_2^{-r} \del_{t_2}
                    \\
                    & = -\1\left( \frac{v_2}{v_1} \right) t_2^{-1} \1\left( \frac{t_2}{t_1} \right)^- \bar{d}v_1 \tensor \del_{v_2} + \del_{v_1}\1\left(\frac{v_2}{v_1}\right) [...] \bar{d}v_1 \tensor \del_{t_2} - \1\left( \frac{v_2}{v_1} \right) t_1^{-1} \bar{d}t_1 \tensor \del_{t_2}
                    \\
                    & = \1\left( \frac{v_2}{v_1} \right) t_2^{-1} \1\left( \frac{t_2}{t_1} \right)^+ \bar{d}v_1 \tensor \del_{v_2} + [...] - \1\left( \frac{v_2}{v_1} \right) t_1^{-1} \bar{d}t_1 \tensor \del_{t_2}
                \end{aligned}
            $$
        \todo[inline]{Dat: Peggy, do you see any way to deal with the $\frac1s$ term in the middle term in the 3rd line ? Also, did I make any computational mistake here ?}
            ...
            \begin{enumerate}
                \item The canonical tensor for the non-degenerate bilinear form $(\cdot, \cdot)_{\extendedtoroidal_A}$ is given explicitly by the following formula:
                    \begin{equation} \label{equation: extended_toroidal_classical_r_matrices}
                        \ddot{\calr}_{\rational} = (\bar{\calr} + [...]) \cdot v_2 \1(v_1 v_2^{-1}) t_2^{-1} \1(t_1/t_2)^+ + [...]
                    \end{equation}
                \item Moreover, the $2$-tensor $\ddot{\calr}_{\rational} \in \extendedtoroidal_A \tensor \extendedtoroidal_A$ as in equation \eqref{equation: extended_toroidal_classical_r_matrices} above satisfies the CYBE \eqref{equation: CYBEs}, i.e. we have:
                    $$\schouten{ \ddot{\calr}_{\rational}, \ddot{\calr}_{\rational} } = 0$$
        ...
        \end{theorem}
            \begin{proof}
                \begin{enumerate}
                    \item Essentially, our job is to combine the $\bbC[v^{\pm 1}, t^{\pm 1}]$-coefficients in the expressions of the basis elements of $\bar{\g} \tensor A, \z_A$, and $\d_A$ (see equations \ref{equation: toroidal_centre_basis} and \eqref{equation: div_zero_vector_fields_basis}).
                    \item 
                \end{enumerate}
            \end{proof}

        \begin{theorem}[Toroidal Lie bialgebras] \label{theorem: toroidal_lie_bialgebras}
            $( \toroidal_A^+, \ddot{\delta}_{\rational}^+|_{\toroidal_A^+} )$ is a Lie sub-bialgebra of $( \extendedtoroidal_A^+, \ddot{\delta}_{\rational}^+ )$.
        \end{theorem}
            \begin{proof}
