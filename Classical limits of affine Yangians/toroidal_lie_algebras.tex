    ...
    \subsection{Lie bialgebra structures on (extended) toroidal Lie algebras} \label{subsection: toroidal_lie_bialgebras}
        ...
        given by:
            \begin{equation} \label{equation: finite_yangian_cobracket}
                \begin{gathered}
                    ...
                    \\
                    \dot{\calr}_{\rational} := \calr \cdot t_2^{-1} \1\left( \frac{t_2}{t_1} \right)^+ = \frac{\calr}{t_2 - t_1}
                \end{gathered}
            \end{equation}
        wherein:
            $$\calr := \calr_{\g, (\g^*)^{\op}}$$
        is the Casimir tensor\footnote{... or in other words, the canonical tensor of the non-degenerate bilinear form $(\cdot, \cdot)_{\Dr(\g)}$ that helps defines the standard Manin triple $( \Dr(\g), \g, (\g^*)^{\op} )$.} of $\g$, and $\1\left( \frac{t_2}{t_1} \right)^+$ is as in equation \eqref{equation: positive_negative_dirac_delta_distributions}.
            ...
            Therefore, the dual cobracket can be given more succinctly by the following formula:
                \begin{equation} \label{equation: finite_dual_yangian_cobracket}
                    \begin{aligned}
                        ...
                        \\
                        & = [\Box(Y), \calr \cdot t_2^{-1} \1\left( \frac{t_2}{t_1} \right)^-]
                    \end{aligned}
        ...
        with $K_{r, s}, c_v, c_t$ as in equation \eqref{equation: toroidal_centre_basis}. In turn, these explicit realisations for $\z_A^{\pm}$ tell us that:
            \begin{equation} \label{equation: positive_and_negative_div_zero_vector_fields}
                \d_A^+ \cong \bigoplus_{(r, s) \in \Z \x \Z_{\geq 0}} \bbC D_{-r, -s + s_0} \oplus \bbC D_t
                \quad, \quad
        ...
        for the $\Z \x \Z_{\geq 0}$-graded Lie algebra $\extendedtoroidal_A^+$, and then a graded-dual basis:
            $$\{ J_{r, s}^{\star} \}_{(r, s) \in \Z \x \Z_{\geq 0}} \subset \extendedtoroidal_A^- \quad, \quad \deg J_{r, s}^{\star} = (-r, -s - 1)$$
            ...
            $$
                \begin{gathered}
                    ...
                    \begin{aligned}
                        ...
                        \\
                        & \cup \left( \{ D_{-r, -s - 1} \}_{(r, s) \in \Z \x \Z_{\geq 0}} \cup \{ D_t \} \right)
                    \end{aligned}
                    \\
                    \{ J_{r, s}^{\star} \}_{(r, s) \in \Z \x \Z_{\geq 0}} := 
                    \begin{aligned}
                        & \{ (x_i v^r t^s)^{\star} = x_i^* v^{-r} t^{-s - 1} \}_{1 \leq i \leq \dim \g, (r, s) \in \Z \x \Z_{\geq 0}}
                        \\
                        & \cup \left( \{ K_{r, s}^{\star} = D_{r, s} \}_{(r, s) \in \Z \x \Z_{\geq 0}} \cup \{ c_v^{\star} = D_v \} \right)
                        \\
                        & \cup \left( \{ D_{-r, -s - 1}^{\star} = K_{-r, -s - 1} \}_{(r, s) \in \Z \x \Z_{\geq 0}} \cup \{ D_t^{\star} = c_t \} \right)
                    \end{aligned}
                \end{gathered}
            $$
            ...
            $$
                \begin{aligned}
                    ...
                    \begin{aligned}
                        ...
                        \\
                        & + \quad \left( \sum_{(r, s) \in \Z \x \Z_{\geq 0}} K_{r, s} \tensor K_{r, s}^{\star} + c_{v_1} \tensor c_{v_2}^{\star} \right) + \quad \left( \sum_{(r, s) \in \Z \x \Z_{\geq 0}} D_{-r, -s - 1} \tensor D_{-r, -s - 1}^{\star} + D_{t_1} \tensor D_{t_2}^{\star} \right)
                    \end{aligned}
                    \\
                    & = 
                    \begin{aligned}
                        & \calr \cdot \1\left( \frac{v_2}{v_1} \right) t_2^{-1} \1\left( \frac{t_2}{t_1} \right)^+
                        \\
                        & + \quad \left( \sum_{(r, s) \in \Z \x \Z_{\geq 0}} K_{r, s} \tensor D_{r, s} + c_{v_1} \tensor D_{v_2} \right) + \quad \left( \sum_{(r, s) \in \Z \x \Z_{\geq 0}} D_{-r, -s - 1} \tensor K_{-r, -s - 1} + D_{t_1} \tensor c_{t_2} \right)
                    \end{aligned}
                \end{aligned}
            $$
        Using the explicit formulae for the elements $K_{r, s}, c_v, c_t$ (cf. equation \eqref{equation: toroidal_centre_basis}) and $D_{r, s}, D_v, D_t$ (cf. equation \eqref{equation: div_zero_vector_fields_basis}), we get\footnote{Recall also that $\1\left( \frac{t_2}{t_1} \right)^- = -\1\left( \frac{t_2}{t_1} \right)^+$ (cf. equation \eqref{equation: positive_negative_dirac_delta_distributions}).}:
            $$
                \begin{aligned}
                    & \sum_{(r, s) \in \Z \x \Z_{\geq 0}} K_{r, s} \tensor D_{r, s}
                    \\
            ...
            \begin{enumerate}
                \item The canonical tensor for the non-degenerate bilinear form $(\cdot, \cdot)_{\extendedtoroidal_A}$ is given explicitly by the following formula:
                    \begin{equation} \label{equation: extended_toroidal_classical_r_matrices}
                        \ddot{\calr}_{\rational} = (\calr + [...]) \cdot v_2 \1(v_1 v_2^{-1}) t_2^{-1} \1(t_1/t_2)^+ + \ddot{\calr}_v + \ddot{\calr}_t
                    \end{equation}
